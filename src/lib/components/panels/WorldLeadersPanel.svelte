...
<Panel id="leaders" title={$tr('panel.leaders')} {count} {loading} {error}>
	{#if leaders.length === 0 && !loading && !error}
		<div class="empty-state">{$tr('common.noData')}</div>
	{:else}
			...
			{#each leaders as leader (leader.id)}
				...
				<div class="leader-card {activityClass}">
					...
					<div class="leader-meta">
						<span class="leader-since">{$tr('common.since', { year: leader.since })}</span>
